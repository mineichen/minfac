...
Following this advice, folders should be named after domain functionality like /cart/add rather than technical descriptions like /persister/cart. Plugin based architecture takes this approach one step further and introduces a dedicated plugin project per feature group. If these projects are integrated as dynamic libraries, features and platform can be released independently as long as everything is built with the same RUSTC version ([https://people.gnome.org/~federico/blog/rust-stable-abi.html](Rust does not have a stable ABI)). Beside features, cross cutting concerns like database access, http-routes, logging or running background tasks could be implemented as reusable plugins too. 

Using a plugin architecture, 
 - Reduces coupling of features to the infrastructure. Files which e.g. know about all http routes or all database migrations violate the [https://de.wikipedia.org/wiki/Open-Closed-Prinzip](open closed principle)
...
This is why I recently released the first version of [https://crates.io/crates/minfac](minfac) on crates.io. Not only does it provide the mentioned features, but it also eliminates weaknesses frequently stumbled on in other languages like [https://docs.microsoft.com/en-us/aspnet/core/fundamentals/dependency-injection?view=aspnetcore-5.0#scope-validation](scope validation). 
...
Creating a microframework on top of minfac is straight forward. The following sections illustrate the takeaways when building the [https://github.com/mineichen/minfac/tree/main/examples/distributed_web](prototype). The workspace has the following structure:
...
# Summary
A plugin based architecture can easily be implemented in Rust using [https://crates.io/crates/minfac](minfac). 
