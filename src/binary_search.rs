...
/// 
pub fn binary_search_by_first_key<'a, T, U, TFn>(input: &'a[T], needle: &U, find: TFn) -> Option<usize> 
    where U: std::cmp::Ord,
        TFn: Fn(&T) -> &U
{
...
pub fn binary_search_by_last_key<'a, T, U, TFn>(input: &'a[T], needle: &U, find: TFn) -> Option<usize> 
    where U: std::cmp::Ord,
        TFn: Fn(&T) -> &U
{
    let mut result_idx = None; 
