...
impl ArcAutoFreePointer {
    pub fn new<T>(i: Arc<T>) -> Self {
        extern "C" fn dropper<T>(i: usize) {
            drop(unsafe { Arc::from_raw(i as *const T) });
        }

        extern "C" fn downgrade<T>(i: usize) -> WeakInfo {
            extern "C" fn drop_weak<T>(i: usize) {
                drop(unsafe { alloc::sync::Weak::from_raw(i as *const T) })
            }

            extern "C" fn strong_count_on_weak<T>(i: usize) -> usize {
...
#[repr(C)]
pub struct WeakInfo {
    weak_ptr: extern "C" fn(usize) -> usize,
    inner: AutoFreePointer,
}
